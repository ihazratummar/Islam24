...
fun MaghribNotification(
    ...
    Box(
        ...
        Column(
            ...
            BasicTopBar(
                topBarTitle = stringResource(R.string.maghrib_notification),
                onBackClick = {
                    onBackClick.invoke()
            ...
            ToggleNotification(
                modifier = Modifier,
                isCheck = notificationState.isMaghribNotification,
                notificationEvent = { notificationEvent(NotificationEvent.ToggleMaghribNotification) },
                notificationName = R.string.maghrib_notification
            )

            AnimatedVisibility(notificationState.isMaghribNotification) {
                AzanList(
                    ...
                    onAzanClick = {url, fileName ->
                        notificationEvent(NotificationEvent.OnMaghribAzanClick(azanUrl = url, fileName = fileName))
                    },
                    listOfAzan = listOfAzan,
                    onSilentNotificationClick = {notificationEvent(NotificationEvent.OnSilentNotificationClick(PrayerName.MAGHRIB, NotificationType.SILENT))},
                    onDefaultNotificationClick = { notificationEvent(NotificationEvent.OnDefaultNotificationClick(PrayerName.MAGHRIB, NotificationType.DEFAULT))},
                    selectedOption = notificationState.selectedMaghribAzan,
                    onOptionSelected = {
                        notificationEvent(NotificationEvent.SelectMaghribAzanOption(it))
                    }
                )
            }
        }
